...
import { DetectionHighlighter } from './DetectionHighlighter.js';
...
export class GraphRenderer {
    constructor(containerId) {
        ...
        // Detection and Timeline modules
    ...
    initializeSVG() {
        ...
        this.detectionHighlighter = new DetectionHighlighter(this);

        // Initialize timeline layout
        this.timelineLayout = new TimelineLayout(this);
    }
    ...
    render(data) {
        ...
        if (this.timelineLayout) {
            this.timelineLayout.forceDeactivate();
        }
        // Also directly remove timeline elements
        ...
        console.log('Nodes:', this.data.nodes.length, 'Links:', this.data.links.length);
        console.log('Sample node:', this.data.nodes[0]);
        console.log('Sample link:', this.data.links[0]);
        console.log('Simulation inner:', this.simulation.simulation);
    ...
    prepareData() {
        ...
        // Resolve link references and filter invalid links
        this.data.links = this.data.links.map(link => {
            const resolved = { ...link };
            if (typeof link.source === 'string') {
                resolved.source = this.data.nodes.find(n => n.id === link.source);
            }
            if (typeof link.target === 'string') {
                resolved.target = this.data.nodes.find(n => n.id === link.target);
            }
            return resolved;
        }).filter(link => {
            if (!link.source || !link.target) {
                console.warn('Filtered out link with unresolved reference:', link);
                return false;
            }
            return true;
        });
    ...
    renderEdges() {
        const link = this.linksGroup
            .selectAll('line')
            .data(this.data.links, d => d ? `${d.source?.id || 'unknown'}-${d.target?.id || 'unknown'}` : Math.random());
    ...
    clearHighlight() {
        ...
        this.linksGroup.selectAll('line')
            .style('opacity', d => d ? ColorScheme.getEdgeOpacity(d) : 0.5);
    }
    ...
    toggleTimeline(options = {}) {
        if (this.timelineLayout) {
            this.timelineLayout.toggle(options);
            this.currentView = this.timelineLayout.isActive ? 'timeline' : 'force';
        }
    }
     ...
     * @param {Object} options - { groupBy, scaleType }
     */
    activateTimeline(options = {}) {
        if (this.timelineLayout && !this.timelineLayout.isActive) {
            this.timelineLayout.activate(options);
            this.currentView = 'timeline';
        }
    }

    /**
     * Deactivate timeline view
     */
    deactivateTimeline() {
        if (this.timelineLayout && this.timelineLayout.isActive) {
     ...
     */
    isDetectionActive() {
        return this.detectionHighlighter?.isActive() || false;
    }
