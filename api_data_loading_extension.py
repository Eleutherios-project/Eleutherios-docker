...
# Configuration
INBOX_DIR = Path(os.environ.get("INBOX_DIR", os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "inbox")))
...
def run_pipeline_background(job_id: str, params: LoadRequest):
    ...
    try:
        ...
        # Advanced mode - not currently needed for wizard, but keeping for API compatibility
            ...
            if params.jsonl_dir:
                cmd = ['python3', 'run_extraction_pipeline_v3.py']
        ...
        # NOTE: Neo4j credentials will be added later to extraction pipeline (Step 2 only)
        # They are NOT needed for document conversion (Step 1)
        ...
        if params.type == 'pdfs' and cmd:
            ...
            if not jsonl_files:
                load_jobs[job_id]['status'] = 'error'
                load_jobs[job_id]['error'] = f'No JSONL files created in {jsonl_output_dir}'
                load_jobs[job_id]['phase'] = 'failed_pdf_conversion'
                logger.error(f"[{job_id}] No JSONL files found in: {jsonl_output_dir}")
                return
            
            # Combine ALL JSONL files into one for processing
            combined_jsonl = jsonl_output_dir / "combined_chunks.jsonl"
            total_chunks = 0
            with open(combined_jsonl, 'w') as outfile:
                for jf in jsonl_files:
                    logger.info(f"[{job_id}] Adding: {jf.name}")
                    with open(jf, 'r') as infile:
                        for line in infile:
                            if line.strip():
                                outfile.write(line)
                                total_chunks += 1
            
            jsonl_output = combined_jsonl
            logger.info(f"[{job_id}] PDF conversion complete: {len(jsonl_files)} JSONL file(s) combined into {total_chunks} chunks")
        ...
        extraction_cmd = [
            'python3', 'run_extraction_pipeline_v3.py',
        ...
        # Override documents with actual file count (not chunks)
        stats['documents'] = load_jobs[job_id].get('file_count', stats.get('documents', 0))
        
        if return_code == 0:
            # STEP 3: Generate embeddings
            load_jobs[job_id]['phase'] = 'embeddings'
            logger.info(f"[{job_id}] STEP 3: Generating embeddings")
            
            try:
                # generate_embeddings.py processes all claims without embeddings
                embedding_cmd = ['python3', 'generate_embeddings.py']
                ...
                embed_process = subprocess.run(
                    ...
                    capture_output=True,
                    text=True,
                    timeout=3600  # 1 hour timeout for embeddings
                )
                
                if embed_process.returncode == 0:
                    logger.info(f"[{job_id}] Embeddings generated successfully")
                    load_jobs[job_id]['output'].append("STEP 3: Embeddings generated successfully")
                else:
                    logger.warning(f"[{job_id}] Embedding generation had issues: {embed_process.stderr[:200]}")
                    load_jobs[job_id]['output'].append(f"STEP 3: Embedding warning: {embed_process.stderr[:200]}")
                    
            except subprocess.TimeoutExpired:
                logger.warning(f"[{job_id}] Embedding generation timed out")
                load_jobs[job_id]['output'].append("STEP 3: Embedding generation timed out")
            except Exception as e:
                logger.warning(f"[{job_id}] Embedding generation error: {e}")
                load_jobs[job_id]['output'].append(f"STEP 3: Embedding error: {str(e)[:100]}")
            
            # Mark as completed regardless of embedding result
            # (claims are still valid, just missing embeddings)
